...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/20 17:59:23 by rbednar           #+#    #+#             */
/*   Updated: 2019/10/15 20:49:09 by ehaggon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_f(t_param list, va_list ap, t_buf *buf)
{
	...
	if (list.length == 'L')
		ft_type_f(list,
				ft_float_f(va_arg(ap, long double), list.precision), buf);
	else
		ft_type_f(list,
				ft_float_f(va_arg(ap, double), list.precision), buf);
}
