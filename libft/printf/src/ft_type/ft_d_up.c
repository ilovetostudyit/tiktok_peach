...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/20 17:47:07 by fcraydle          #+#    #+#             */
/*   Updated: 2019/10/15 20:47:46 by ehaggon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	minus_pos(char *i, t_param list, int tmp, t_buf *buf)
{
	...
	*i == '-' ? tmp_2 = tmp - 1 : 0;
	...
	if (list.precision > tmp_2 && list.zero == 0)
		ft_buf_add_s(buf, ft_memnew_pf(list.precision - tmp_2, '0'), 0);
	ft_buf_add_s(buf, i, 0);
	if (list.width > list.precision && list.width > tmp && list.zero == 1)
		ft_buf_add_s(buf, ft_memnew_pf(list.width - list.space - list.plus -
...
static void	prec_neg(char *i, t_param list, int tmp, t_buf *buf)
{
	...
	if (list.minus == 1 && list.width > tmp)
		ft_buf_add_s(buf, ft_memnew_pf(list.width - tmp - list.space, ' '), 0);
}
