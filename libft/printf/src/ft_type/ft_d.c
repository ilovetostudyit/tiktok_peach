...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/20 17:46:54 by fcraydle          #+#    #+#             */
/*   Updated: 2019/10/19 20:08:10 by rbednar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	prec_neg(char *i, t_param list, int tmp, t_buf *buf)
{
	...
	if (list.minus == 0 && list.zero == 0 && list.width > tmp)
		ft_buf_add_s_d(buf, ft_memnew_pf(list.width - tmp -
	...
	if (list.minus == 1 && list.width > tmp)
		ft_buf_add_s_d(buf, \
		ft_memnew_pf(list.width - tmp - list.space, ' '), 0);
}
...
{
	if (list.length == 0)
		ft_type_d(list, ft_itoa_base(va_arg(ap, int), 10), buf);
	else if (list.length == 'l')
		ft_type_d(list, ft_itoa_base(va_arg(ap, long), 10), buf);
	else if (list.length == 'l' + 'l')
		ft_type_d(list,
					ft_itoa_base(va_arg(ap, long long), 10), buf);
